...
## Playground - быстрое тестирование с горячей перезагрузкой

Для разработки и тестирования анимации без установки расширения:

```bash
# Запустить playground с HMR
pnpm run playground

# Откроется http://localhost:5173/playground/
# Любые изменения в src/content/index.js тут же отражаются в браузере
```

**Особенности playground:**
- ✅ Полная работа WebGPU/fallback2D анимации
- ✅ Горячая перезагрузка (HMR) - тестируйте изменения без перезагрузки
- ✅ Все параметры снега настраиваются через UI
- ✅ Смена тем (light/dark) для тестирования логики свечения
- ✅ Не требует установки расширения в браузер
